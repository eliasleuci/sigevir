    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div>
          ...
          <p className="text-sm text-gray-500 font-medium">Registro inmutable de todas las acciones críticas realizadas en el sistema.</p>
        </div>
          ...
          <table className="w-full text-left">
            <thead>
              <tr className="bg-white border-b border-gray-50">
                ...
                <th className="px-8 py-5 text-[10px] font-black text-gray-400 uppercase tracking-widest">Acción / Evento</th>
                <th className="px-8 py-5 text-[10px] font-black text-gray-400 uppercase tracking-widest">Objeto</th>
            ...
            <tbody className="divide-y divide-gray-50">
              {logs.map((log) => (
                <tr key={log.id} className="hover:bg-gray-50/50 transition-colors">
                  ...
                  <td className="px-8 py-5">
                    <p className="text-xs font-black text-gray-700 uppercase">{log.Usuario?.nombre || 'SISTEMA'}</p>
                  </td>
                  <td className="px-8 py-5">
                  ...
                  <td className="px-8 py-5">
                    <p className="text-xs font-black text-blue-600">{log.dominio || '-'}</p>
                  </td>
                  <td className="px-8 py-5">
                    <p className="text-xs text-gray-500 font-medium italic">"{log.observaciones}"</p>
                  </td>
                </tr>
              ))}
              {logs.length === 0 && (
                <tr>
                  <td colSpan={5} className="px-8 py-20 text-center">
                    <div className="flex flex-col items-center">
                      ...
                      <p className="text-gray-400 italic">No hay registros de auditoría que mostrar.</p>
                    </div>
                  </td>
                </tr>
              )}
