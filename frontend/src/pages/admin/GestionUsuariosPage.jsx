...
const DEMO_USERS_ADMIN = [
  { id: 'demo-admin', nombre_completo: 'Admin Demo', email: 'admin@sigevir.demo', dni: '10.000.001', rol: 'admin', institucion: 'Policia de Cordoba', activo: true, created_at: new Date().toISOString() },
  { id: 'demo-campo', nombre_completo: 'Oficial Perez', email: 'campo@sigevir.demo', dni: '20.000.002', rol: 'agente_campo', institucion: 'Policia de Cordoba', activo: true, created_at: new Date().toISOString() },
  { id: 'demo-deposito', nombre_completo: 'Juan Deposito', email: 'deposito@sigevir.demo', dni: '30.000.003', rol: 'deposito', institucion: 'Deposito Municipal', activo: true, created_at: new Date().toISOString() },
  { id: 'demo-fiscal', nombre_completo: 'Dra. Rodriguez', email: 'fiscal@sigevir.demo', dni: '40.000.004', rol: 'fiscal_juez', institucion: 'Fiscalia de Cordoba', activo: true, created_at: new Date().toISOString() },
];
...
const GestionUsuariosPage = () => {
  ...
  const fetchUsuarios = useCallback(async () => {
    ...
    try {
      if (!SUPABASE_READY) {
        ...
        if (filtroEstado === 'inactivo') data = data.filter(function(u) { return !u.activo; });
        if (search) {
          var s = search.toLowerCase();
          data = data.filter(function(u) { return u.nombre_completo.toLowerCase().indexOf(s) >= 0 || u.email.toLowerCase().indexOf(s) >= 0; });
        }
        setUsuarios(data);
        setLoading(false);
        return;
  ...
  const openEditModal = (usuario) => {
    ...
    const tipoPersonal = TIPOS_PERSONAL_PUBLICOS.find(t => t.rol === usuario.rol);
    setFormData({
      ...
      cargo: usuario.cargo || '',
      tipo_personal_id: String(tipoPersonal?.id || ''),
      password: '',
      confirmar_password: '',
    });
    setShowModal(true);
  ...
  const handleSave = async (e) => {
    e.preventDefault();
    if (!formData.nombre_completo || !formData.dni || !formData.email || !formData.tipo_personal_id) {
      toast.error('Completá los campos obligatorios');
      return;
    }
    setFormLoading(true);
    try {
      const tipoPersonal = TIPOS_PERSONAL_PUBLICOS.find(t => t.id === Number(formData.tipo_personal_id));
      ...
      if (editingUser) {
        const { error } = await supabase
          ...
          .update({
            ...
            tipo_personal_id: Number(formData.tipo_personal_id),
            rol,
          })
      ...
      <div className="bg-white rounded-[32px] border border-gray-100 shadow-xl overflow-hidden">
        <div className="p-5 border-b border-gray-50 bg-gray-50/30 flex flex-col sm:flex-row gap-3 items-start sm:items-center justify-between">
          <div className="relative w-full max-w-xs">
            ...
            <input value={search} onChange={(e) => { setSearch(e.target.value); setPage(1); }} type="text" placeholder="Buscar por nombre, email, DNI..." className="w-full pl-10 pr-4 py-2 bg-white border border-gray-200 rounded-xl text-sm outline-none focus:ring-2 focus:ring-blue-100" />
          </div>
          <div className="flex gap-2 w-full sm:w-auto">
            <select value={filtroRol} onChange={(e) => { setFiltroRol(e.target.value); setPage(1); }} className="px-3 py-2 bg-white border border-gray-200 rounded-xl text-sm outline-none focus:ring-2 focus:ring-blue-100">
              <option value="">Todos los roles</option>
              {Object.entries(ROLES).map(([key, label]) => (<option key={key} value={key}>{label}</option>))}
            </select>
            <select value={filtroEstado} onChange={(e) => { setFiltroEstado(e.target.value); setPage(1); }} className="px-3 py-2 bg-white border border-gray-200 rounded-xl text-sm outline-none focus:ring-2 focus:ring-blue-100">
                ...
                <tr key={u.id} className="hover:bg-gray-50/50 transition-colors group">
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-3">
                      <div className={`${ROLE_COLORS[u.rol]?.bg || 'bg-gray-100'} w-9 h-9 rounded-xl flex items-center justify-center text-sm font-bold ${ROLE_COLORS[u.rol]?.text || 'text-gray-600'}`}>
                  ...
                  <td className="px-6 py-4 text-sm font-medium text-gray-600">{u.dni}</td>
                  <td className="px-6 py-4">{getRoleBadge(u.rol)}</td>
                  <td className="px-6 py-4"><p className="text-xs font-semibold text-gray-500">{u.institucion}</p></td>
                  <td className="px-6 py-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="col-span-2">
                    ...
                    <select value={formData.tipo_personal_id} onChange={(e) => setFormData(p => ({ ...p, tipo_personal_id: e.target.value }))} className="w-full px-3 py-2 border border-gray-200 rounded-xl text-sm outline-none focus:ring-2 focus:ring-blue-100 bg-white">
                      ...
                      {[...TIPOS_PERSONAL_PUBLICOS, { id: 7, nombre: 'Administrador', rol: 'admin' }].map((t) => (
